# Talk AI Documentation

Start with the root [README](../README.md) for the project overview.

## Overview

- [FEATURES.md](./FEATURES.md): Current product surface and user-facing behavior
- [ARCHITECTURE.md](./ARCHITECTURE.md): Services, data model, and runtime flows
- [DEVELOPMENT.md](./DEVELOPMENT.md): Build, test, migration, and local verification notes

## Setup And Operations

- [QUICK_START.md](./QUICK_START.md)
- [BOT_SETUP_GUIDE.md](./BOT_SETUP_GUIDE.md)
- [TROUBLESHOOTING.md](./TROUBLESHOOTING.md)
- [WEBHOOK_DEBUG_GUIDE.md](./WEBHOOK_DEBUG_GUIDE.md)

## Feature Guides

- [ONBOARDING.md](./ONBOARDING.md)
- [pseudo_streaming.md](./pseudo_streaming.md)
- [RAG_TOOL_GUIDE.md](./RAG_TOOL_GUIDE.md)
- [RAG_BACKGROUND_JOBS_GUIDE.md](./RAG_BACKGROUND_JOBS_GUIDE.md)
- [CATALOGUE_TOOL_GUIDE.md](./CATALOGUE_TOOL_GUIDE.md)

## Analyses And Audits

- [SECURITY_AUDIT.md](./SECURITY_AUDIT.md)
- [SMART_PICKER_BOT_PROVIDER.md](./SMART_PICKER_BOT_PROVIDER.md)

## Project

- [CODE_OF_CONDUCT.md](./CODE_OF_CONDUCT.md)

